...
import (
	...
	"math"
	"sync"
	"testzilla/modules/http_factory"
	"time"
)
...
func (s *StreamReport) Collect(records <-chan *http_factory.ReportRecord) {
	latencyWithinSecTemp := &Stats{}
	go func() {
		ticker := time.NewTicker(time.Second)
		lastCount := int64(0)
		lastTime := http_factory.StartTime
		for {
			select {
			case <-ticker.C:
				s.lock.Lock()
...
func (s *StreamReport) Snapshot() *SnapshotReport {
	...
	rs := &SnapshotReport{
		Elapsed: time.Since(http_factory.StartTime),
